...
Raspberry Pi (Blinka), wired through the EYESPI Pi Beret.

adafruit_epd is the framebuf path (with optional external SRAM offload) and runs
under Blinka on Linux, where displayio is unavailable. For a Feather (native
CircuitPython), use feather_epd_grayscale_2in13_breakout.py instead.
...
Wiring — #4197 EYESPI Breakout → EYESPI Pi Beret → Raspberry Pi:
  CS   → CE0 (GPIO8)    DC   → GPIO25
  RST  → GPIO27         BUSY → GPIO17
  SCK → SCK   MOSI → MOSI   MISO → MISO   3V3 → 3V3   GND → GND

Note: the Beret routes DC to GPIO25. The all-in-one 2.13" eInk Pi Bonnet (#4687)
routes DC to GPIO22 instead — see rpi_epd_grayscale_2in13_bonnet.py for that board.
...
# Raspberry Pi (Blinka) + EYESPI Pi Beret
spi = busio.SPI(board.SCK, MOSI=board.MOSI, MISO=board.MISO)
cs = digitalio.DigitalInOut(board.CE0)
dc = digitalio.DigitalInOut(board.D25)  # Beret routes DC to GPIO25
rst = digitalio.DigitalInOut(board.D27)
busy = digitalio.DigitalInOut(board.D17)

# 2.13" Breakout #4197 (FPC-7528B). colstart=0; it is only nonzero on panels
# with a physical left dead-zone (e.g. MagTag FPC-7519, colstart=8).
